...
class CTOSIntegrationController extends Controller
{
    ...
    public function index(){
        ...
        return view('osc.integration.ctos.index');
    }
    ...
    public function view($id){
        ...
        return view('osc.integration.ctos.view',
        compact('integrate','responseDecode')
        );
    }

    //{{--Working Code Datatable with indexNo--}}
    public function CTOSLogDatatable(Request $request){
        
        $now = Carbon::now()->format('Y-m-d');

        $query = IntegrationLog::where('ILType','CTOS')->whereDate('ILCD',$now)->orderby('ILID','desc')->get();
        ...
        return datatables()->of($query)
            ...
            ->editColumn('ILNo', function($row) {
                ...
                $route = route('osc.integration.ctos.view',[$row->ILNo] );
