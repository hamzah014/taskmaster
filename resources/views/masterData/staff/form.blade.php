                ...
                <div class="col s10 m6 l6 breadcrumbs-left">
                    ...
                    @else
                        ...
                        <ol class="breadcrumbs mb-0">
                            ...
                            <li class="breadcrumb-item active">{{ __('New Staff') }}
                            </li>
                        </ol>
                    @endif
                </div>
                            ...
                            <div class="card-content">
                                ...
                                @isset($staff)
                                    <form class="ajax-form" novalidate
                                        action="{{ route('masterData.staff.update', [$staff->ESID]) }}" method="POST">
                                    @else
                                        <form class="ajax-form" novalidate action="{{ route('masterData.staff.store') }}"
                                            method="POST">
                                        @endisset
                                        @csrf
                                        <div class="row">
                                            <div class="col s12">
                                                <div class="row">
                                                    <div class="input-field col m8 s6 form-group">
                                                        ...
                                                        {!! Form::text('name', $staff->ESName ?? null, ['id' => 'name']) !!}
                                                ...
                                                <div class="row">
                                                    <div class="input-field col m4 s6 form-group">
                                                        ...
                                                        {!! Form::text('email', $staff->ESEmail ?? null, ['id' => 'email', 'class' => 'form-control']) !!}
                                                        <label for="email">{{ __('Email') }}<span
                                                                style="color:red">*</span> </label>
                                                    </div>
                                                    <div class="input-field col m4 s6 form-group">
                                                        <i class="material-icons prefix">phone_iphone</i>
                                                        {!! Form::text('phone', $staff->ESPhoneNo ?? null, ['id' => 'phone', 'class' => 'form-control']) !!}
                                                ...
                                                <div class="row">
                                                    <div class="input-field col m4 s6 form-group">
                                                        ...
                                                        @if ($currentUser == 'EMBADMIN' || $currentUser == 'EMBSTAFF')
                                                            <input type="hidden" name="embassy" id="embassyCode"
                                                                value="{{ $staff->ES_EMCode ?? null }}">
                                                        @else
                                                            <input type="hidden" name="embassy" id="getEmbassy"
                                                                value="{{ $staff->ES_EMCode ?? null }}">
                                                        @endif
                                                        {!! Form::select('', $embassies, $staff->ES_EMCode ?? null, [
                                                    ...
                                                    <div class="input-field col m4 s6 form-group">
                                                        @if ($currentUser == 'EMBADMIN' || $currentUser == 'EMBSTAFF')
                                                            <input type="hidden" name="role" id="embassyRole"
                                                                value="{{ $staff->user->role->RLCode ?? null }}">
                                                        @else
                                                            <input type="hidden" name="role" id="getRole"
                                                                value="{{ $staff->user->role->RLCode ?? null }}">
                                                        @endif
                                                        <i class="material-icons prefix">expand_circle_down</i>
                                                        {!! Form::select('', $roles, $staff->user->role->RLCode ?? null, [
                                                            'class' => 'select2 form-control',
                                                            'id' => 'role',
                                                            'placeholder' => trans('message.dropdown_default'),
                                                        ]) !!}
                                                    ...
                                                    <div class="input-field col m4 s6 form-group">
                                                        ...
                                                        {!! Form::select('isActive', $isActive, $user->USActive ?? null, [
                                                            'id' => 'isActive',
                                                            'class' => 'select2 form-control',
                                                            'placeholder' => trans('message.dropdown_default'),
                                                        ]) !!}
...
@push('script')
    <script type="text/javascript">
        ...
        @if ($currentUser == 'EMBADMIN')
            ...
            // Set value to role if create
            $('#role').val('EMBSTAFF');

            // Set value to embassy if create
            @if (Route::currentRouteName() == 'masterData.staff.create')
                $('#embassy').val('{{ $user->ES_EMCode }}');
                $('#embassyCode').val('{{ $user->ES_EMCode }}');
                $('#embassyRole').val('EMBSTAFF');
            @elseif (Route::currentRouteName() == 'masterData.staff.edit')
                $('#embassy').val('{{ $staff->ES_EMCode }}');
            @endif
